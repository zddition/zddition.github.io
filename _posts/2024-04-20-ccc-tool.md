...
function highlightDifferences() {
    ...
    pairs.forEach(pair => {
        const [_, angleStr] = pair.split(":");
        const angleValue = parseInt(angleStr, 10);
        if (!isNaN(angleValue) && angleValue >= 1 && angleValue <= 360) {
            inputAngles.push(angleValue);
        }
    });

    // Sort input angles
    inputAngles.sort((a, b) => a - b);

    // Calculate differences and highlight cells
    for (let i = 0; i < inputAngles.length; i++) {
        const diff = inputAngles[i + 1] - inputAngles[i];
        if (!isNaN(diff)) {
            const row = Math.floor((inputAngles[i] - 1) / 20);
            const col = (inputAngles[i] - 1) % 20;

            if (row < 18 && col < 19) { // Check boundary
                table.rows[row].cells[col].style.border = `2px solid black`;
                table.rows[row].cells[col + 1].style.border = `2px solid black`;
                table.rows[row + 1].cells[col].style.border = `2px solid black`;
                table.rows[row + 1].cells[col + 1].style.border = `2px solid black`;
            }
        }
    }
}
